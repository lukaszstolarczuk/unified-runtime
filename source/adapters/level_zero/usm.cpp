    ...
    case UR_STRUCTURE_TYPE_USM_POOL_LIMITS_DESC: {
      ...
      for (auto &config : DisjointPoolConfigs.Configs) {
        if (umfDisjointPoolParamsSetMaxPoolableSize(config, Limits->maxPoolableSize) != UMF_RESULT_SUCCESS) {
          logger::error("urUSMPoolCreate: setting maxPoolableSize in DisjointPool params failed");
        ...
        if (umfDisjointPoolParamsSetSlabMinSize(config, Limits->minDriverAllocSize) != UMF_RESULT_SUCCESS) {
          logger::error("urUSMPoolCreate: setting slabMinSize in DisjointPool params failed");
          throw UsmAllocationException(UR_RESULT_ERROR_UNKNOWN);
        }
      }
      break;
